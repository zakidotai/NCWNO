...
def eval_gradients_per_output(forward_fn, x, label, mu_params_grad, pbar_outputs=None, 
                              max_outputs=None, chunk_size=100):
    """
    Compute squared gradients using per-output-element gradient computation.
    
    OPTIMIZED VERSION: 
    - Uses torch.autograd.grad instead of backward() for better efficiency
    - Supports sampling a subset of outputs to dramatically speed up computation
    - Processes in chunks to balance memory and speed
    
    This avoids the cancellation problem that occurs when summing outputs before backward.
    ...
    Args:
        ...
        max_outputs: Maximum number of output elements to process (None = all).
                     If specified, randomly samples this many outputs for speed.
        chunk_size: Number of output elements to process in each batch (default: 100)
    ...
    # Sample outputs if max_outputs is specified (for speed)
    if max_outputs is not None and max_outputs < num_outputs:
        # Randomly sample indices
        indices = torch.randperm(num_outputs, device=output_flat.device)[:max_outputs]
        output_flat = output_flat[indices]
        num_outputs_processed = max_outputs
        print(f'  Sampling {max_outputs} out of {num_outputs} outputs for speed')
    else:
        indices = None
    ...
    if pbar_outputs is not None:
        pbar_outputs.reset(total=num_outputs_processed)

    # Process outputs in chunks
    # Using torch.autograd.grad is more efficient than backward() 
    # because it doesn't need to store intermediate gradients
    for i in range(num_outputs_processed):
        # Compute gradient for this single output element
        # Using torch.autograd.grad is faster than backward()
        grad_i = torch.autograd.grad(
            outputs=output_flat[i],
            inputs=mu_params_grad,
            grad_outputs=torch.tensor(1.0, device=output_flat.device, dtype=output_flat.dtype),
            retain_graph=True,
            create_graph=False,
            only_inputs=True
        )[0]
        ...
        grads_squared_sum += grad_i ** 2
        
        # Update progress bar
        if pbar_outputs is not None:
            pbar_outputs.update(1)

    # Mean over output elements (unbiased estimate via sampled outputs)
    grads_squared_mean = grads_squared_sum / num_outputs_processed
...
def compute_sensitivity(data_loader, forward_fn, mu_params, sigma_params,
                        ...
                        use_per_output=True, max_outputs=None):
    """
    ...
    Args:
        ...
        use_per_output: If True (default), compute gradients per output element to avoid
                       ...
                       cancellation issues).
        max_outputs: Maximum number of output elements to process per batch (None = all).
                     If specified, randomly samples this many outputs for dramatic speedup.
                     Recommended: 1000-5000 for 50x-100x speedup with minimal accuracy loss.
    ...
    for batch_idx, (xx, yy) in enumerate(pbar_batches):
        ...
        if use_per_output:
            ...
            grads_squared = eval_gradients_per_output(forward_fn, xx, label, mu_params_grad, 
                                                       pbar_outputs, max_outputs=max_outputs)
...
def get_num_sensitive_from_saved_data(saved_data, var_threshold, pde_idx=None):
    """
    Helper function to compute number of sensitive parameters for any variance threshold
    from saved sensitivity analysis results.
    
    Args:
        saved_data: Dictionary loaded from saved .pt file
        var_threshold: Variance threshold (0.0 to 1.0)
        pde_idx: Index of PDE to analyze (None = combined/averaged across all PDEs)
                 If specified, analyzes that specific PDE (0, 1, 2, ...)
    
    Returns:
        num_sensitive: Number of parameters explaining var_threshold variance
        sensitive_indices: Indices of sensitive parameters (sorted)
    """
    if pde_idx is not None:
        # Analyze specific PDE
        if 'per_pde_sorted_data' in saved_data and pde_idx < len(saved_data['per_pde_sorted_data']):
            pde_data = saved_data['per_pde_sorted_data'][pde_idx]
            cumulative_var_frac = pde_data['cumulative_variance_fraction']
            sorted_indices = pde_data['sorted_indices']
            num_sensitive = torch.sum(cumulative_var_frac <= var_threshold).item() + 1
            sensitive_indices = sorted_indices[:num_sensitive].sort()[0]
        else:
            # Fallback: recompute from raw sensitivity scores
            if 'per_pde_sensitivity' not in saved_data:
                raise ValueError(f"PDE {pde_idx} data not found in saved_data")
            sensitivity_scores = saved_data['per_pde_sensitivity'][pde_idx]
        ...
        # Analyze combined (averaged across all PDEs)
        if 'cumulative_variance_fraction' in saved_data:
            # Use precomputed cumulative variance fraction (faster)
            cumulative_var_frac = saved_data['cumulative_variance_fraction']
            sorted_indices = saved_data['sorted_indices']
            num_sensitive = torch.sum(cumulative_var_frac <= var_threshold).item() + 1
            sensitive_indices = sorted_indices[:num_sensitive].sort()[0]
        else:
            # Fallback: recompute from raw sensitivity scores
            sensitivity_scores = saved_data['combined_sensitivity']
            total_var = torch.sum(sensitivity_scores)
            sorted_sens, sorted_indices = torch.sort(sensitivity_scores, descending=True)
            cumulative_var = torch.cumsum(sorted_sens, 0)
            num_sensitive = torch.sum(cumulative_var / total_var <= var_threshold).item() + 1
...
def verify_forward_function(model, forward_fn, mu_params, x_sample, label, T0, step, T, device):
    ...
    for module in model.modules():
        if hasattr(module, "sampling"):
            # sampling_states.append((module, module.sampling))
...
def main(args):
    ...
    for case_idx in range(len(data)):
        ...
        sensitivity_scores = compute_sensitivity(
            ...
            max_outputs=args.max_outputs  # Sample outputs for speed
        )
    ...
    # Precompute sorted scores and cumulative variance for easy post-processing
    # This allows computing num_sensitive for any variance threshold without re-sorting
    
    # For combined (averaged across all PDEs)
    total_var = torch.sum(combined_sensitivity)
    sorted_sens, sorted_indices = torch.sort(combined_sensitivity, descending=True)
    cumulative_var = torch.cumsum(sorted_sens, 0)
    cumulative_var_fraction = cumulative_var / total_var  # Normalized to [0, 1]
    
    # For each PDE individually
    per_pde_sorted_data = []
    for pde_idx, pde_sensitivity in enumerate(all_sensitivity_scores):
        pde_total_var = torch.sum(pde_sensitivity)
        pde_sorted_sens, pde_sorted_indices = torch.sort(pde_sensitivity, descending=True)
        pde_cumulative_var = torch.cumsum(pde_sorted_sens, 0)
        pde_cumulative_var_fraction = pde_cumulative_var / pde_total_var
        
        per_pde_sorted_data.append({
            'sorted_sensitivity': pde_sorted_sens,
            'sorted_indices': pde_sorted_indices,
            'cumulative_variance': pde_cumulative_var,
            'cumulative_variance_fraction': pde_cumulative_var_fraction,
            'total_variance': pde_total_var.item(),
            'pde_name': data_paths[pde_idx].split("/")[-1] if pde_idx < len(data_paths) else f'PDE_{pde_idx}'
        })
    ...
    # Save sensitivity scores with precomputed values for post-processing
    save_path = os.path.join(output_dir, f'sensitivity_scores_ncwno_maxoutputs_{args.max_outputs}_nbatches_{args.num_batches}.pt')
    torch.save({
        # Raw sensitivity scores (for any analysis)
        'combined_sensitivity': combined_sensitivity,
        'per_pde_sensitivity': all_sensitivity_scores,
        
        # Precomputed sorted values for COMBINED (averaged across all PDEs)
        'sorted_sensitivity': sorted_sens,  # Sensitivity scores sorted descending
        'sorted_indices': sorted_indices,   # Indices that sort the scores
        'cumulative_variance': cumulative_var,  # Cumulative sum of sorted scores
        'cumulative_variance_fraction': cumulative_var_fraction,  # Normalized [0, 1]
        'total_variance': total_var.item(),  # Total variance for normalization
        
        # Precomputed sorted values for EACH PDE (for per-PDE post-processing)
        'per_pde_sorted_data': per_pde_sorted_data,  # List of dicts, one per PDE
        
        # Results for the specific threshold used
        'sensitive_indices': sensitive_indices,
        'num_sensitive': num_sensitive,
        'var_threshold': args.var_threshold,  # Threshold used during computation
        ...
        'num_pdes': len(all_sensitivity_scores)
    }, save_path)
    print(f'\nSensitivity scores saved to: {save_path}')
    print(f'\nPost-processing: Results saved with precomputed sorted values for:')
    print(f'  - Combined (averaged across all PDEs)')
    print(f'  - Each PDE individually')
    print(f'\n  Examples:')
    print(f'    from sens_ncwno_data_fast import get_num_sensitive_from_saved_data')
    print(f'    import torch')
    print(f'    data = torch.load("{save_path}")')
    print(f'    ')
    print(f'    # Combined analysis:')
    print(f'    num_sensitive, indices = get_num_sensitive_from_saved_data(data, 0.95)')
    print(f'    ')
    print(f'    # Per-PDE analysis (PDE 0, 1, 2, ...):')
    print(f'    for pde_idx in range(data["num_pdes"]):')
    print(f'        num_sensitive, indices = get_num_sensitive_from_saved_data(data, 0.95, pde_idx=pde_idx)')
    print(f'        pde_name = data["per_pde_sorted_data"][pde_idx]["pde_name"]')
    print(f'        print(f"PDE {{pde_name}}: {{num_sensitive}} sensitive parameters")')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_outputs', type=int, default=None,
                        help='Maximum number of output elements to process per batch '
                             '(None = all). Sampling outputs dramatically speeds up computation. '
                             'Recommended: 1000-5000 for 50x-100x speedup with minimal accuracy loss.')
