...
class VIHMCTrainer_NCWNO:
    ...
    def define_model_log_prob(
        ...
        # Pre-create label tensor once (avoids repeated allocation)
        _label_tensor = torch.zeros(1, device=device)

        def _compute_batch_loss(weights, x_batch, y_batch):
            """Compute log-likelihood for a single batch.
            
            Separated so it can be wrapped with gradient checkpointing.
            """
            temp_y = self.functional_model(weights, (x_batch, _label_tensor))
            y_dev = y_batch.to(device)
            temp_y = temp_y.reshape(y_dev.shape)

            if model_loss == "regression":
                return -0.5 * tau_out * ((temp_y - y_dev) ** 2).sum(0)
            elif model_loss == "binary_class_linear_output":
                crit = nn.BCEWithLogitsLoss(reduction="sum")
                return -tau_out * crit(temp_y, y_dev)
            elif model_loss == "multi_class_linear_output":
                crit = nn.CrossEntropyLoss(reduction="sum")
                return -tau_out * crit(temp_y, y_dev.long().view(-1))
            elif model_loss == "multi_class_log_softmax_output":
                return -tau_out * torch.nn.functional.nll_loss(
                    temp_y, y_dev.long().view(-1)
                )
            elif model_loss == "NLL":
                return -nll_loss(temp_y, y_dev, tau_out * torch.ones_like(temp_y))
            elif callable(model_loss):
                return -model_loss(temp_y, y_dev).sum(0)
            else:
        ...
        def log_prob_func(params):
            ...
            weights = self.mean_params.clone().to(device)
            weights[self.sens_indices] = params

            if predict:
                # In predict mode (called under torch.no_grad), accumulate outputs
                output = []
                y = []
                for x_batch, y_batch in tr_data:
                    temp_y = self.functional_model(
                        weights, (x_batch.to(device), _label_tensor)
                    )
                ...
                # Training mode: compute loss per-batch with gradient checkpointing
                # to keep only ONE batch's activations in GPU memory at a time
                # (instead of ALL batches simultaneously).
                ll = torch.tensor(0.0, device=device)
                for x_batch, y_batch in tr_data:
                    x_dev = x_batch.to(device)
                    # Gradient checkpointing: frees intermediate activations
                    # during forward; recomputes them during backward.
                    batch_ll = grad_checkpoint(
                        _compute_batch_loss, weights, x_dev, y_batch,
                        use_reentrant=False
                    )
                    ll = ll + batch_ll
                    del x_dev, batch_ll
...
def build_sensitivity_json(results_dir, output_json, var_thresholds=None):
    """
    Process all sensitivity .pt files from results_dir and build a JSON file.

    The JSON structure is:
    {
        "<maxout>_<nbatches>": {
            "<var_threshold>": {
                "combined": [list of sensitive parameter indices],
                "pde_0": [list of sensitive parameter indices],
                "pde_1": [...],
                ...
            },
            ...
    ...
    print(f"Found {len(files_to_process)} sensitivity .pt files in {results_dir}/")

    for i, (fname, maxout, nbatch) in enumerate(files_to_process, 1):
        filepath = os.path.join(results_dir, fname)
        print(f"[{i}/{len(files_to_process)}] Processing: {fname}")

        data = torch.load(filepath, map_location="cpu", weights_only=False)
        key = f"{maxout}_{nbatch}"
        sensitivity_db[key] = {}
        ...
        for vt in var_thresholds:
            ...
            if "cumulative_variance_fraction" in data:
                cvf = data["cumulative_variance_fraction"]
                n_sensitive = int(torch.sum(cvf <= vt).item()) + 1
                top_indices = data["sorted_indices"][:n_sensitive]
                entry["combined"] = sorted(top_indices.tolist())
            elif "combined_sensitivity" in data:
                cs = data["combined_sensitivity"]
                total = cs.sum()
                sorted_s, sorted_idx = torch.sort(cs, descending=True)
                cum = torch.cumsum(sorted_s, 0) / total
                n_sensitive = int(torch.sum(cum <= vt).item()) + 1
                entry["combined"] = sorted(sorted_idx[:n_sensitive].tolist())
            ...
            if "per_pde_sorted_data" in data:
                for pde_idx in range(num_pdes):
                    ...
                    cvf = pde_data["cumulative_variance_fraction"]
                    n_sensitive = int(torch.sum(cvf <= vt).item()) + 1
                    top_indices = pde_data["sorted_indices"][:n_sensitive]
                    entry[f"pde_{pde_idx}"] = sorted(top_indices.tolist())
            elif "per_pde_sensitivity" in data:
                for pde_idx, pde_sens in enumerate(data["per_pde_sensitivity"]):
                    total = pde_sens.sum()
                    sorted_s, sorted_idx = torch.sort(pde_sens, descending=True)
                    cum = torch.cumsum(sorted_s, 0) / total
                    n_sensitive = int(torch.sum(cum <= vt).item()) + 1
                    entry[f"pde_{pde_idx}"] = sorted(sorted_idx[:n_sensitive].tolist())

            sensitivity_db[key][vt_key] = entry

    with open(output_json, "w") as f:
        json.dump(sensitivity_db, f)

    print(f"\nSensitivity JSON written to: {output_json}")
    print(f"  Keys: {list(sensitivity_db.keys())}")
    return sensitivity_db


def load_sensitivity_indices(json_path, maxout, nbatches, var_threshold, pde_index=None):
    """
    Load sensitivity indices from a pre-built JSON file.

    :param json_path: Path to the sensitivity JSON file.
    :param maxout: Max outputs value used during sensitivity computation.
    :param nbatches: Number of batches used during sensitivity computation.
    :param var_threshold: Variance threshold (e.g. 0.9).
    :param pde_index: PDE index (0, 1, 2, ...). If None, uses combined indices.
    :return: List of sensitive parameter indices.
    """
    with open(json_path, "r") as f:
        sensitivity_db = json.load(f)

    key = f"{maxout}_{nbatches}"
    vt_key = str(var_threshold)

    if key not in sensitivity_db:
        available = list(sensitivity_db.keys())
        raise KeyError(
            f"Key {key} not found in {json_path}. "
            f"Available keys: {available}"
        )

    if vt_key not in sensitivity_db[key]:
        available = list(sensitivity_db[key].keys())
        raise KeyError(
            f"Variance threshold {vt_key} not found for key {key}. "
            f"Available thresholds: {available}"
        )

    entry = sensitivity_db[key][vt_key]

    if pde_index is not None:
        pde_key = f"pde_{pde_index}"
        if pde_key not in entry:
            available = [k for k in entry.keys() if k.startswith("pde_")]
            raise KeyError(
                f"PDE index {pde_key} not found. Available: {available}"
            )
        indices = entry[pde_key]
    else:
        indices = entry.get("combined", [])

    print(f"Loaded {len(indices)} sensitive indices from {json_path}")
    print(f"  key={key}, var_threshold={var_threshold}, "
          f"pde={'combined' if pde_index is None else pde_index}")
    return indices
...
def main(args):
    ...
    # Build or load sensitivity JSON
    sensitivity_json = args.sensitivity_json
    if args.build_json or not os.path.exists(sensitivity_json):
        print(f'\nBuilding sensitivity JSON from {args.results_dir}...')
        build_sensitivity_json(
            args.results_dir, sensitivity_json, var_thresholds=[0.8, 0.9, 0.95]
        )

    # Load sensitivity indices for the specified maxout/nbatches/pde
    sensitivity_indices = load_sensitivity_indices(
        json_path=sensitivity_json,
        maxout=args.maxout,
        nbatches=args.nbatches,
        var_threshold=args.var_threshold,
        pde_index=args.pde_index,
    )
    print(f'  Number of sensitive parameters: {len(sensitivity_indices)}')
    ...
    for case_idx in range(len(data)):
        ...
        params_hmc = vihmc_trainer.run(train_loaders[case_idx], device=device)
        results[case_idx] = params_hmc

    return results, sensitivity_indices
...
if __name__ == '__main__':
    ...
    parser.add_argument('--maxout', type=str, required=True,
                        help='Max outputs value used during sensitivity computation '
                             '(e.g. 100, 500, 1000)')
    parser.add_argument('--nbatches', type=int, required=True,
                        help='Number of batches used during sensitivity computation '
                             '(e.g. 2, 5, 10, 20, 50, 100)')
    parser.add_argument('--pde_index', type=int, default=None,
                        help='PDE index for per-PDE sensitivity (0, 1, 2, ...). '
                             'If not specified, uses combined sensitivity across all PDEs.')
    parser.add_argument('--sensitivity_json', type=str,
                        default='results_sensitivity/sensitivity_indices.json',
                        help='Path to sensitivity indices JSON file')
    parser.add_argument('--results_dir', type=str,
                        default='results_sensitivity',
                        help='Directory containing sensitivity .pt files')
    parser.add_argument('--build_json', action='store_true',
                        help='Force rebuild of the sensitivity JSON from .pt files')

    args = parser.parse_args()

    results, sensitive_indices = main(args)

    # save results as a .pt file
    torch.save(results, f'results/results_hmc_ncwno_1d_{args.maxout}_{args.nbatches}_{args.var_threshold}_{args.pde_index}.pt')
    torch.save(sensitive_indices, f'results/sensitive_indices_ncwno_1d_{args.maxout}_{args.nbatches}_{args.var_threshold}_{args.pde_index}.pt')